<script setup>
import { ref, computed } from 'vue';
import { useRouter } from 'vue-router';
import AlunoService from '@/services/AlunoService.js';

const router = useRouter();
const loading = ref(false);
const error = ref(null);
const success = ref(false);

const aluno = ref({
  nome: '',
  cpf: '',
  dataNascimento: '',
  sexo: 'Masculino',
  endereco: '',
  telefone: '',
  problemasSaude: '',
  faixa: 'Branca',
  plano: 'Plano Ouro (3x)',
  vencimento: '',
  responsavelNome: '',
  responsavelCpf: '',
  faixasHistorico: [] // Inicializado como array vazio
});

// Lógica para verificar menor de idade
const isMenorDeIdade = computed(() => {
  if (!aluno.value.dataNascimento) return false;
  const hoje = new Date();
  const nasc = new Date(aluno.value.dataNascimento);
  let idade = hoje.getFullYear() - nasc.getFullYear();
  const mes = hoje.getMonth() - nasc.getMonth();
  if (mes < 0 || (mes === 0 && hoje.getDate() < nasc.getDate())) idade--;
  return idade < 18;
});

const faixas = ['Branca', 'Cinza', 'Amarela', 'Laranja', 'Verde', 'Azul', 'Roxa', 'Marrom', 'Preta', 'Coral', 'Vermelha'];
const planos = ['Plano Bronze (1x)', 'Plano Prata (2x)', 'Plano Ouro (3x)'];

function adicionarFaixa() {
  aluno.value.faixasHistorico.push({ 
    faixa: aluno.value.faixa || 'Branca', 
    dataObtencao: new Date().toISOString().slice(0,10), 
    grau: 0 
  });
}

function removerFaixa(index) {
  aluno.value.faixasHistorico.splice(index, 1);
}

function marcarComoAtual(index) {
  const h = aluno.value.faixasHistorico[index];
  if (!h) return;
  aluno.value.faixa = h.faixa;
}

async function handleSubmit() {
  loading.value = true;
  error.value = null;
  try {
    // Enviando o objeto completo para o AlunoService
    await AlunoService.createAluno(aluno.value);
    success.value = true;
    // Redireciona após 2 segundos
    setTimeout(() => router.push('/alunos'), 2000);
  } catch (err) {
    // Captura a mensagem de erro vinda do backend se disponível
    error.value = err.response?.data?.message || 'Erro ao cadastrar. Verifique a conexão com o servidor.';
  } finally {
    loading.value = false;
  }
}
</script>

<template>
  <h1 class="display-5 mb-4">Matricular Novo Aluno</h1>

  <div class="card border-0 shadow-sm">
    <div class="card-body">
      
      <div v-if="success" class="alert alert-success">Aluno matriculado com sucesso!</div>
      <div v-if="error" class="alert alert-danger">{{ error }}</div>

      <form v-if="!success" @submit.prevent="handleSubmit">
        
        <h5 class="mb-3 text-secondary border-bottom pb-2">Dados Pessoais</h5>
        <div class="row g-3 mb-4">
          <div class="col-md-6">
            <label class="form-label">Nome Completo *</label>
            <input type="text" v-model="aluno.nome" class="form-control" required>
          </div>
          <div class="col-md-3">
            <label class="form-label">CPF</label>
            <input type="text" v-model="aluno.cpf" class="form-control" placeholder="000.000.000-00">
          </div>
          <div class="col-md-3">
            <label class="form-label">Data de Nascimento *</label>
            <input type="date" v-model="aluno.dataNascimento" class="form-control" required>
          </div>
          <div class="col-md-3">
            <label class="form-label">Sexo</label>
            <select v-model="aluno.sexo" class="form-select">
              <option value="Masculino">Masculino</option>
              <option value="Feminino">Feminino</option>
            </select>
          </div>
          <div class="col-md-5">
            <label class="form-label">Endereço Completo</label>
            <input type="text" v-model="aluno.endereco" class="form-control">
          </div>
          <div class="col-md-4">
            <label class="form-label">Telefone / WhatsApp</label>
            <input type="text" v-model="aluno.telefone" class="form-control">
          </div>
        </div>

        <div v-if="isMenorDeIdade" class="p-3 mb-4 bg-light rounded border border-warning">
          <h5 class="mb-3 text-warning">Dados do Responsável (Aluno Menor de Idade)</h5>
          <div class="row g-3">
            <div class="col-md-8">
              <label class="form-label">Nome do Responsável *</label>
              <input type="text" v-model="aluno.responsavelNome" class="form-control" :required="isMenorDeIdade">
            </div>
            <div class="col-md-4">
              <label class="form-label">CPF do Responsável *</label>
              <input type="text" v-model="aluno.responsavelCpf" class="form-control" :required="isMenorDeIdade">
            </div>
          </div>
        </div>

        <h5 class="mb-3 text-secondary border-bottom pb-2">Saúde</h5>
        <div class="mb-4">
          <label class="form-label">Problemas de Saúde / Alergias / Observações</label>
          <textarea v-model="aluno.problemasSaude" class="form-control" rows="2"></textarea>
        </div>

        <h5 class="mb-3 text-secondary border-bottom pb-2">Dados da Matrícula</h5>
        <div class="row g-3 mb-4">
          <div class="col-md-4">
            <label class="form-label">Faixa Inicial</label>
            <select v-model="aluno.faixa" class="form-select">
              <option v-for="f in faixas" :key="f" :value="f">{{ f }}</option>
            </select>
          </div>
          <div class="col-md-4">
            <label class="form-label">Plano</label>
            <select v-model="aluno.plano" class="form-select">
              <option v-for="p in planos" :key="p" :value="p">{{ p }}</option>
            </select>
          </div>
          <div class="col-md-4">
            <label class="form-label">1º Vencimento</label>
            <input type="date" v-model="aluno.vencimento" class="form-control" required>
          </div>
        </div>

        <h5 class="mb-3 text-secondary border-bottom pb-2">Histórico de Faixas</h5>
        <div class="mb-4">
          <div v-for="(h, idx) in aluno.faixasHistorico" :key="idx" class="row g-3 align-items-end mb-2">
            <div class="col-md-4">
              <label class="form-label">Faixa</label>
              <select v-model="h.faixa" class="form-select">
                <option v-for="f in faixas" :key="f" :value="f">{{ f }}</option>
              </select>
            </div>
            <div class="col-md-4">
              <label class="form-label">Data de Obtenção</label>
              <input type="date" v-model="h.dataObtencao" class="form-control">
            </div>
            <div class="col-md-2">
              <label class="form-label">Grau</label>
              <input type="number" min="0" v-model="h.grau" class="form-control">
            </div>
            <div class="col-md-2">
              <button type="button" @click="removerFaixa(idx)" class="btn btn-outline-danger btn-sm">Remover</button>
            </div>
          </div>
          <button type="button" @click="adicionarFaixa" class="btn btn-outline-primary btn-sm mt-2">Adicionar Faixa ao Histórico</button>
        </div>

        <div class="d-flex justify-content-end gap-2">
          <button type="button" @click="router.back()" class="btn btn-outline-secondary">Cancelar</button>
          <button type="submit" class="btn btn-danger" :disabled="loading">
            {{ loading ? 'Salvando...' : 'Realizar Matrícula' }}
          </button>
        </div>
      </form>
    </div>
  </div>
</template>